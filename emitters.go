...
func (b *byGroupsEmitter) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	for {
		token, err := d.Token()
		if err != nil {
			return err
		...
		case xml.StartElement:
			...
			b.Emitters = append(b.Emitters, emitter)
...
func (b *byGroupsEmitter) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	name := xml.Name{Local: "bygroups"}
	if err := e.EncodeToken(xml.StartElement{Name: name}); err != nil {
		return err
	}
	for _, m := range b.Emitters {
...
// TODO: We currently have no way of looking up a Lexer while deserialising, due
//   to circular imports.
