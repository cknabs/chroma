...
var (
	...
	emitterTemplates   = func() map[string]SerialisableEmitter {
		...
		for _, emitter := range []SerialisableEmitter{
			...
			&usingSelfEmitter{},
			TokenType(0),
			// TODO Can't be supported until the lexer registry is refactored to avoid cycles.
			// &byGroupNamesEmitter{},
			// &usingEmitter{},
		} {
...
func MustNewXMLLexer(config *Config, from fs.FS, path string) *RegexLexer {
	lex, err := NewXMLLexer(config, from, path)
	if err != nil {
		panic(err)
	}
	return lex
...
func NewXMLLexer(config *Config, from fs.FS, path string) (*RegexLexer, error) {
	if config == nil {
		config = &Config{}
	}
...
func Unmarshal(data []byte) (*RegexLexer, error) {
	...
	lex, err := NewLazyLexer(&root.Config, func() Rules { return root.Rules })
	if err != nil {
		return nil, err
	}
	return lex, nil
